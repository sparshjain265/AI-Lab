	...
	def astar(self, env, heu):
		...
		while(self.Q):
			...
			if(env.topRight()):
				...
				if(self.visited[env.x][env.y] == -1):
					self.visited[env.x][env.y] = 1
					heapq.heappush(self.Q, (env.heuristic(heu, counter), [env.x, env.y, self.p + 1, 5]))
				env.bottomLeft()
			if(env.topLeft()):
				counter += 1
				if(env.percept() == True):
				...
				if(self.visited[env.x][env.y] == -1):
					self.visited[env.x][env.y] = 1
					heapq.heappush(self.Q, (env.heuristic(heu, counter), [env.x, env.y, self.p + 1, 6]))
				env.bottomRight()
			if(env.bottomLeft()):
				counter += 1
				if(env.percept() == True):
				...
				if(self.visited[env.x][env.y] == -1):
					self.visited[env.x][env.y] = 1
					heapq.heappush(self.Q, (env.heuristic(heu, counter), [env.x, env.y, self.p + 1, 7]))
				env.topRight()
			if(env.bottomRight()):
				counter += 1
				if(env.percept() == True):
				...
				if(self.visited[env.x][env.y] == -1):
					self.visited[env.x][env.y] = 1
					heapq.heappush(self.Q, (env.heuristic(heu, counter), [env.x, env.y, self.p + 1, 8]))
				env.topLeft()
